...
export async function getAuthContext() {
  ...
  return {
    user,
    profile: profile ? ({ ...profile, vendor_name: vendorName } as Profile) : null
  };
}

export async function requireAuth(
  roles?: AppRole[]
): Promise<{ user: NonNullable<Awaited<ReturnType<typeof getAuthContext>>["user"]>; profile: NonNullable<Awaited<ReturnType<typeof getAuthContext>>["profile"]> }> {
  ...
  return ctx as {
    ...
    profile: NonNullable<Awaited<ReturnType<typeof getAuthContext>>["profile"]>;
  };
}

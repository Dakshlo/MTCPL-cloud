-- ──────────────────────────────────────────────────────────────────
-- Migration 018: finish_block_cut() — single-RPC Cutting Done.
-- ──────────────────────────────────────────────────────────────────
-- The Cutting Done flow used to do 18+ sequential Supabase round-
-- trips, which kept timing out on Vercel and leaving partial-commit
-- state (some slabs cut_done, cut_session_block still cutting).
-- Even after parallelising the JS action it stayed flaky on the
-- floor.
--
-- This function does the entire flow inside one PG transaction:
--   • Idempotency pre-flight (skip slabs already cut_done from this
--     block — safe to re-run on retry)
--   • Insert remainder blocks (skip duplicates)
--   • Mark parent block consumed
--   • Mark planned slabs cut_done / open as the operator chose
--   • Mark unplanned extras cut_done with this block as source
--   • Transfer slabs from donor blocks: edit donor layout, remove
--     cut_session_slabs links, mark donors needs_reprint, then
--     update transferred slab requirements
--   • Mark cut_session_block done
--
-- Notify + audit_log writes still happen from the JS action AFTER
-- this returns — they're not part of the atomic critical path.
--
-- Returns JSONB with the new state so the JS action can use it for
-- audit metadata + revalidatePath.
-- ──────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF array_length(v_pending_xfer, 1) > 0 THEN
    ...
    -- One UPDATE statement walks every donor; jsonb_path_query rebuilds
    -- the placed array filtered by id NOT IN our pending xfer set.
...
-- Grant execute to authenticated users — RLS isn't bypassed because the
-- function is SECURITY DEFINER and we already validate auth in the
-- calling JS action via requireAuth().

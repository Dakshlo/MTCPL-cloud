-- ──────────────────────────────────────────────────────────────────────
-- Migration 141 — finish_block_cut: fix the overload split
-- ──────────────────────────────────────────────────────────────────────
-- Bug (confirmed on prod — `SELECT oid::regprocedure FROM pg_proc WHERE
-- proname='finish_block_cut'` returns TWO rows):
--
--   • mig 018 created finish_block_cut as a 12-arg function.
--   • mig 020 DROPped the 12-arg and recreated it 13-arg (added the trailing
--     `p_stock_location TEXT DEFAULT NULL`).
--   • mig 034 / 035 / 131 were each written as 12-arg CREATE OR REPLACE.
--     A different argument count cannot replace the 13-arg function, so those
--     three SPAWNED A SECOND (12-arg) overload instead of updating the one
--     that runs. Result: two overloads coexist.
--   • approveCutAction calls the RPC WITH p_stock_location
--     (src/app/(app)/cutting/actions.ts ~1198-1211), so PostgREST resolves to
--     the 13-arg MIG-020 body — which predates and therefore LACKS:
--        – mig 034: deleting stale cut_session_slabs rows for not-cut slabs
--        – mig 035: cut_source_kind tagging ('planned'/'extra'/'transferred')
--        – mig 131: the "never regress an advanced slab" status guard on
--                   Step 3 (a pre-cut slab already in carving/dispatch was
--                   being dragged back to cut_done on final approval)
--     i.e. the last three fixes have been DORMANT in production.
--
-- Fix:
--   1. CREATE OR REPLACE the 13-ARG signature (the one approveCutAction hits)
--      with the mig-131 guarded body, merged with mig-020's stock_location
--      stamping — a strict superset (mig-034 + mig-035 + mig-131 + mig-020).
--      Because the signature now MATCHES the live 13-arg function, this finally
--      replaces the body that actually executes.
--   2. DROP the now-redundant 12-arg overload so a single canonical function
--      remains. Safe: the only caller passes p_stock_location, and the 13-arg
--      keeps `DEFAULT NULL`, so even a 12-param-style call resolves here.
--
-- Safety: function definition only; no row data is touched at migration time.
-- This does NOT repair any slab already mis-regressed by the old body — that
-- is a separate data question (none known beyond the OM-0026/OM-0033 re-link,
-- which was a payload-drop, not this bug).
-- Rollback: re-run mig 020 (13-arg) — but that reintroduces the dormancy.
-- ──────────────────────────────────────────────────────────────────────

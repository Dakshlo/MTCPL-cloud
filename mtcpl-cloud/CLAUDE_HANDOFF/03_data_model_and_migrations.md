...
| 029 | `enable_rls_on_public_tables.sql` | Defence-in-depth: enables RLS + adds `authenticated_read_all` SELECT policy on every public-schema table in one loop. Closes the long-standing `rls_disabled_in_public` Supabase advisor warning. Anon key can no longer SELECT directly; server actions (admin client) keep working unchanged; client-side realtime/notification reads keep working via the authenticated-read policy. **Any future migration that adds a public table MUST include its own `ENABLE ROW LEVEL SECURITY` + read policy** (as 028 does) or this migration must be re-run. |
| 041 | `inventory_scaffolding.sql` | **Inventory module — Scaffolding (v1).** Adds `storekeeper` to `app_role`; three new enums `inventory_movement_type` (issue/return/receive/writeoff/transfer/adjust), `inventory_movement_status` (pending_approval/approved/rejected/cancelled), `scaffolding_component_type` (standard/ledger/transom/brace/jack_base/u_head/coupler/plank/ladder/toe_board/tie_rod/other); three new tables `sites` (singleton `is_plant=TRUE` row pre-seeded; project-site rows added through UI), `scaffolding_components` (catalog pre-seeded with 18 standard variants — five Standard sizes, four Ledger sizes, two Transoms, plus single-size Brace / Jack Base / U-Head / Coupler / Plank / Ladder / Toe Board), `inventory_movements` (append-only ledger, one row per (component × qty) per batch). Stock-on-hand is DERIVED (no materialised table) — `loadInventorySnapshot()` does the SELECT + JS reduce. Storekeeper proposes every movement; crosscheck (Mafat) or owner approves via `/inventory/approvals`. Read-only RLS, admin-client writes. Partial indexes for pending-batch queue + per-site/per-component queries. No post-migration UPDATE required; assigning the storekeeper role to a profile is optional and done from `/users`. |

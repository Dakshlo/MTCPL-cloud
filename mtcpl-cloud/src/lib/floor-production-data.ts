 ...
 *  and one approaching it. And next, vendor-wise numbers of all 3
 *  vendors in one page."
 *
 * Two slides, one query. Both read APPROVED carving output — a slab
 * counts on the day the carving head signed it off (review_approved_at),
 * not when the operator unloaded it. That is the same event the CNC
 * monthly report and the WhatsApp daily report bill on, so the wall can
 * never disagree with the paperwork.
 *
 ...
 * unit. Daksh asks for this in CFT and the floor says CFT, so the slides
 * label it CFT — but it is the combined figure, and mixing the two is
...
export type FloorVendorNumbers = {
  vendorId: string;
  name: string;
  /** Combined units approved this month so far. */
  thisMonth: number;
  /** Same, for the whole of last month. */
  lastMonth: number;
  /** Same-day-of-month point in last month — the fair comparison. */
...
export async function buildFloorProductionData(): Promise<FloorProduction> {
  ...
  const [items, { data: vendorRows }, { data: machineRows }] = await Promise.all([
    ...
    fetchAllPaged<{
      ...
      admin
        .from("carving_items")
        .select("slab_requirement_id, vendor_id, review_approved_at, carving_sides")
    ...
    admin
      .from("cnc_machines")
      .select("id, vendor_id, status, is_active")
      .eq("is_active", true),
  ]);
  ...
  type VAgg = { thisMonth: number; lastMonth: number; lastMonthToDate: number; slabs: number };
  const byVendor = new Map<string, VAgg>();
  ...
  for (const it of items) {
    ...
    if (!vid) continue;
    const agg = byVendor.get(vid) ?? { thisMonth: 0, lastMonth: 0, lastMonthToDate: 0, slabs: 0 };
    if (isThis) {
      agg.thisMonth += u;
      agg.slabs += 1;
    } else if (isPrev) {
      agg.lastMonth += u;
      // Same day-of-month cut, so a vendor's "vs last month" is like
      // for like on the 9th as much as on the 30th.
      if (p.d <= today) agg.lastMonthToDate += u;
    }
  ...
  const machinesByVendor = new Map<string, { total: number; carving: number; idle: number }>();
  for (const mrow of (machineRows ?? []) as Array<{ vendor_id: string; status: string | null }>) {
    const g = machinesByVendor.get(mrow.vendor_id) ?? { total: 0, carving: 0, idle: 0 };
    g.total += 1;
    if (mrow.status === "carving") g.carving += 1;
    else if (mrow.status !== "maintenance" && mrow.status !== "inactive") g.idle += 1;
    machinesByVendor.set(mrow.vendor_id, g);
  }

  const vendors: FloorVendorNumbers[] = ((vendorRows ?? []) as Array<{ id: string; name: string }>)
    .map((v) => {
      const a = byVendor.get(v.id) ?? { thisMonth: 0, lastMonth: 0, lastMonthToDate: 0, slabs: 0 };
      const mc = machinesByVendor.get(v.id) ?? { total: 0, carving: 0, idle: 0 };
      return {
        vendorId: v.id,
        name: v.name,
        thisMonth: a.thisMonth,
        lastMonth: a.lastMonth,
        lastMonthToDate: a.lastMonthToDate,
        slabs: a.slabs,
        machines: mc.total,
        carving: mc.carving,
        idle: mc.idle,

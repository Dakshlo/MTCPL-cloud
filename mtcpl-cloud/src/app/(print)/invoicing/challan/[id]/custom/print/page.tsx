...
export default async function CustomBillPrintPage({ params }: { params: Params }) {
  ...
  const totals = computeInvoiceTotals(amounts, { mode: gstMode, igst: Number(c.igst_percent) || 0, cgst: Number(c.cgst_percent) || 0, sgst: Number(c.sgst_percent) || 0 });
  const totalTax = gstMode === "igst" ? totals.igstAmt : gstMode === "cgst_sgst" ? totals.cgstAmt + totals.sgstAmt : 0;
  const gstLabel = gstMode === "igst" ? `IGST @ ${Number(c.igst_percent) || 0}%` : gstMode === "cgst_sgst" ? `CGST + SGST @ ${Number(c.cgst_percent) || 0}% + ${Number(c.sgst_percent) || 0}%` : "—";
          ...
          <>
            ...
            <div className="totbox">
              ...
              <div className="totals">
                ...
                {gstMode === "igst" && <div className="row alt"><span>IGST @ {Number(c.igst_percent) || 0}%</span><span className="mono">{rupee(totals.igstAmt)}</span></div>}
                {gstMode === "cgst_sgst" && (<><div className="row alt"><span>CGST @ {Number(c.cgst_percent) || 0}%</span><span className="mono">{rupee(totals.cgstAmt)}</span></div><div className="row alt"><span>SGST @ {Number(c.sgst_percent) || 0}%</span><span className="mono">{rupee(totals.sgstAmt)}</span></div></>)}

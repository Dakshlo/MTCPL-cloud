...
// Dispatch Storage client — park-all (one-click clear of Make Dispatch) + a
// searchable, collapsible per-temple list of parked slabs with bring-back.

import { useMemo, useState, type CSSProperties } from "react";
import { useRouter } from "next/navigation";
import { parkAllReadyDispatchAction, unparkDispatchSlabsAction } from "../actions";
...
export function DispatchStorageClient({ parked, readyCount }: { parked: ParkedSlab[]; readyCount: number }) {
  ...
  function toggleOpen(t: string) { setOpen((p) => { const n = new Set(p); n.has(t) ? n.delete(t) : n.add(t); return n; }); }
  ...
  function toggleTemple(slabs: ParkedSlab[]) {
    ...
    setSelected((p) => { const n = new Set(p); for (const s of slabs) allOn ? n.delete(s.id) : n.add(s.id); return n; });
  }
  ...
  async function parkAll() {
    ...
    try {
      const res = await parkAllReadyDispatchAction();
      if (!res.ok) setErr(res.error);
      else { setMsg(`Moved ${res.count} slab${res.count === 1 ? "" : "s"} to storage. Make Dispatch is now clear.`); router.refresh(); }
  ...
  async function bringBack(ids: string[]) {
    ...
    const confirmMsg = ids.length === 1 ? `Bring slab ${ids[0]} back to Make Dispatch?` : `Bring ${ids.length} slabs back to Make Dispatch?`;
    if (!window.confirm(confirmMsg)) return;
    setBusy(true); setErr(null); setMsg(null);
    try {
      const res = await unparkDispatchSlabsAction(ids);
      if (!res.ok) setErr(res.error);
      else { setMsg(`Brought ${res.count} slab${res.count === 1 ? "" : "s"} back to Make Dispatch.`); setSelected(new Set()); router.refresh(); }
    } catch { setErr("Failed — check your connection."); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800 }}>In Make Dispatch: <span style={{ color: readyCount > 0 ? "#b45309" : "#15803d" }}>{readyCount}</span></div>
          <div className="muted" style={{ fontSize: 12.5, marginTop: 2 }}>Move the whole ready list into storage in one click, then bring back only the few you need.</div>
        </div>
        {confirmPark ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 12.5, fontWeight: 700 }}>Move all {readyCount}?</span>
          ...
          <button type="button" disabled={busy || readyCount === 0} onClick={() => setConfirmPark(true)} style={{ ...btn("#b45309"), opacity: readyCount === 0 ? 0.5 : 1, cursor: readyCount === 0 ? "not-allowed" : "pointer" }}>
            🗄 Move all {readyCount} ready → storage
          </button>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search parked slabs — code, label, temple, stone, size…" style={{ flex: "1 1 280px", padding: "9px 12px", fontSize: 13.5, border: "1px solid var(--border)", borderRadius: 9, background: "var(--bg)", color: "var(--text)" }} />
      ...
      {parked.length === 0 ? (
        <div className="banner">Storage is empty. Use “Move all ready → storage” above, or send selected slabs from the dispatch picker.</div>
      ) : filtered.length === 0 ? (

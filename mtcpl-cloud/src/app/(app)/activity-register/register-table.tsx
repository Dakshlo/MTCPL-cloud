...
// How the activity was communicated / handed over.
const REFERENCE_OPTIONS = ["Email", "WhatsApp", "Hand to hand"];
...
export function ActivityRegisterTable({
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) =>
      [e.code, e.date, fmtDate(e.date), e.activity, e.person, e.concernPerson, e.reference]
        .filter(Boolean)
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
        <input
          ...
          placeholder="Search code, activity, person, concern person, reference, date…"
          style={{ flex: "1 1 320px", padding: "9px 14px", fontSize: 14, border: "1px solid var(--border)", borderRadius: 10, background: "var(--bg)", color: "var(--text)" }}
        />
        <button
          type="button"
          onClick={() => setModal({ mode: "new" })}
          style={{ padding: "9px 18px", fontSize: 14, fontWeight: 800, color: "#fff", background: "var(--gold-dark)", border: "none", borderRadius: 10, cursor: "pointer", whiteSpace: "nowrap" }}
        >
          ＋ New entry
        </button>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 1000 }}>
          <thead>
            <tr style={{ background: "var(--surface-alt, rgba(0,0,0,0.03))", textAlign: "left" }}>
              {["Sr", "Code", "Date", "Activity", "Person", "Concern person", "Reference", "Proof", ""].map((h, i) => (
                <th key={i} style={{ padding: "10px 12px", fontSize: 11, fontWeight: 800, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.04em", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={9} style={{ padding: 28, textAlign: "center", color: "var(--muted)" }}>
                  {q ? "No entries match your search." : "No entries yet. Tap “＋ New entry” to add the first record."}
                </td>
              </tr>
            ) : (
              filtered.map((e) => (
                <tr key={e.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  <td style={{ padding: "9px 12px", fontFamily: "ui-monospace, monospace", color: "var(--muted)" }}>{e.srNo}</td>
                  <td style={{ padding: "9px 12px", fontFamily: "ui-monospace, monospace", fontWeight: 700, whiteSpace: "nowrap" }}>{e.code}</td>
                  <td style={{ padding: "9px 12px", whiteSpace: "nowrap" }}>{fmtDate(e.date)}</td>
                  <td style={{ padding: "9px 12px", minWidth: 240, whiteSpace: "pre-wrap" }}>{e.activity}</td>
                  <td style={{ padding: "9px 12px" }}>{e.person || "—"}</td>
                  <td style={{ padding: "9px 12px" }}>{e.concernPerson || "—"}</td>
                  <td style={{ padding: "9px 12px", whiteSpace: "nowrap" }}>{e.reference || "—"}</td>
                  <td style={{ padding: "9px 12px", whiteSpace: "nowrap" }}>
                    {e.hasProof ? (
                      <a href={`/api/activity-register/proof/${e.id}`} target="_blank" rel="noopener noreferrer" style={{ color: "var(--gold-dark)", fontWeight: 700, textDecoration: "none" }}>
                        📎 View
                      </a>
                    ) : (
...
function EntryModal({
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(15,23,42,0.5)", backdropFilter: "blur(3px)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "5vh 16px", overflowY: "auto" }}
    >
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 520, background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 16, padding: 22, boxShadow: "0 24px 60px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <h2 style={{ margin: 0, fontSize: 18 }}>{isEdit ? `Edit ${entry?.code}` : "New entry"}</h2>
        ...
        {!isEdit && (
          <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 10 }}>
            {site.name} · code <strong style={{ fontFamily: "ui-monospace, monospace", color: "var(--text)" }}>{site.nextCode}</strong>
          </div>
        )}
        <form
        ...
        >
          ...
          <Field label="Date">
            <input name="activity_date" type="date" defaultValue={isEdit ? entry!.date : todayISO()} style={inputStyle} />
          </Field>
          <Field label="Activity *">
            <textarea
              ...
              placeholder="e.g. Sent black-granite demo sample (2 pieces)"
          ...
          <Field label="Reference (sent via)">
            <select name="reference" defaultValue={isEdit ? entry!.reference : ""} style={inputStyle}>
              <option value="">—</option>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 4 }}>
            ...
            <button type="submit" style={{ padding: "9px 20px", fontSize: 13, fontWeight: 800, color: "#fff", background: "var(--gold-dark)", border: "none", borderRadius: 8, cursor: "pointer" }}>{isEdit ? "Save changes" : "Add entry"}</button>
          </div>
        </form>
      </div>
    </div>

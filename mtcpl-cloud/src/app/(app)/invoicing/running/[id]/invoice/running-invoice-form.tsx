...
/** Price a running challan → invoice (Daksh, mig 182). Split: running challan
 *  iframe LEFT, item tables (fixed particulars/qty, editable RATE) + GST RIGHT.
 *  "Convert to invoice" posts convertRunningToInvoiceAction; 👁 Preview shows the
 *  full tax invoice (BulkInvoicePreview, NOT VALID watermark). */
...
export function RunningInvoiceForm({ id, code, temple, editMode, sourceDispatchId, initSections, initGst, initDiscount, roundTotal = false, bill, ship, invLabel }: {
    ...
    <div style={{ display: "flex", gap: 16, alignItems: "flex-start", paddingBottom: 40 }}>
      ...
      <div style={{ flex: "1 1 500px", minWidth: 360, position: "sticky", top: 10, display: "flex", flexDirection: "column", height: "calc(100vh - 20px)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", background: "var(--surface)", marginTop: 44 }}>
        <div style={{ padding: "9px 14px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <span style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--muted)" }}>🏃 Running challan — {code}</span>
          <Link href={`/invoicing/challan/${id}/running/print`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11.5, fontWeight: 700, color: "var(--gold-dark)", textDecoration: "none", whiteSpace: "nowrap" }}>Open full ↗</Link>
        </div>
        <iframe src={`/invoicing/challan/${id}/running/print`} title="Running challan" style={{ flex: 1, width: "100%", border: "none", background: "#f0f0f0" }} />
      </div>

      {/* RIGHT — pricing form. */}
      <div style={{ flex: "1 1 520px", minWidth: 360, marginTop: 44 }}>
        <Link href="/invoicing/dropped" style={{ fontSize: 12, fontWeight: 600, color: "var(--muted)", textDecoration: "none" }}>← Running bills</Link>
        ...
        <p style={{ fontSize: 12.5, color: "var(--muted)", margin: "0 0 16px" }}>🏛 {temple} · add a rate for each line + GST. Items come from the running challan.</p>

        <form action={convertRunningToInvoiceAction}>

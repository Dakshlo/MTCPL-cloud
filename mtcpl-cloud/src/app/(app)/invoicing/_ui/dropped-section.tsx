...
export function DroppedSection({ dropped, showHeader = true, initialEditId }: { dropped: DroppedChallan[]; showHeader?: boolean; initialEditId?: string }) {
  // ?edit=<id> (from the Invoices page ✎ Edit) auto-opens the bill form for
  // that challan — in EDIT mode when it's already billed/invoiced (Jul 2026).
  const [billing, setBilling] = useState<DroppedChallan | null>(() => dropped.find((d) => d.id === initialEditId) ?? null);
    ...
    <div style={{ marginTop: showHeader ? 24 : 0 }}>
      {showHeader && (
        <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.05em", color: "#5b21b6", marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
          🏃 Running bills — custom whole-piece <span style={pill}>{dropped.length}</span>
        </div>
      )}
      {dropped.length === 0 ? (
        <div style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: 12, padding: "30px 22px", textAlign: "center", color: "var(--muted)" }}>
          No running bills. On the <strong>Challans</strong> page, drag a challan onto the <strong>🏃 Running bill</strong> drop zone.
        </div>
      ) : (
      ...
      {billing && <BillForm d={billing} edit={billing.customBilled} onClose={() => setBilling(null)} />}
    ...
    <div style={{ border: "1px solid var(--border)", borderLeft: "4px solid #8b5cf6", borderRadius: 12, background: "var(--surface, #fff)", padding: "12px 13px", display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        ...
        {d.customBilled
          ...
          : <span style={{ fontSize: 10, fontWeight: 800, color: "#92400e", background: "rgba(245,158,11,0.16)", borderRadius: 999, padding: "2px 9px" }}>🏃 RUNNING BILL</span>}
      </div>
...
function BillForm({ d, edit = false, onClose }: { d: DroppedChallan; edit?: boolean; onClose: () => void }) {
  const [items, setItems] = useState<Item[]>(() =>
    edit && d.items.length
      ? d.items.map((it) => ({ particulars: it.particulars, hsn: it.hsn, unit: it.unit, quantity: it.quantity ? String(it.quantity) : "", rate: it.rate ? String(it.rate) : "" }))
      : [blank()],
  );
  ...
  return (
    <div onMouseDown={onClose} style={overlay}>
      <form action={createCustomBillAction} onMouseDown={(e) => e.stopPropagation()} style={{ width: "min(760px, 100%)", background: "var(--surface, #fff)", borderRadius: 14, padding: 18, boxShadow: "0 24px 60px rgba(0,0,0,0.35)", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <input type="hidden" name="edit_mode" value={edit ? "1" : ""} />
        ...
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, marginBottom: 4, flexWrap: "wrap" }}>
          <div style={{ fontSize: 16, fontWeight: 800 }}>{edit ? "Edit bill" : "Custom bill"} — {d.code} <span className="muted" style={{ fontSize: 13, fontWeight: 600 }}>· 🏛 {d.temple}</span>{edit && <span style={{ marginLeft: 8, fontSize: 10, fontWeight: 800, color: "#6d28d9", background: "rgba(124,58,237,0.1)", borderRadius: 999, padding: "2px 8px" }}>NUMBER LOCKED</span>}</div>
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 16, flexWrap: "wrap", alignItems: "center" }}>
          <button type="submit" disabled={!hasItems} style={{ ...btnPrimary, background: hasItems ? "#6d28d9" : "var(--border)", cursor: hasItems ? "pointer" : "default" }}>{edit ? "💾 Save changes (number unchanged)" : "🧾 Create custom bill & deliver"}</button>
    ...
    <div onMouseDown={onClose} style={overlay}>
      <form action={convertCustomBillToInvoiceAction} onMouseDown={(e) => e.stopPropagation()} style={{ width: "min(440px, 100%)", background: "var(--surface, #fff)", borderRadius: 16, padding: 20, boxShadow: "0 24px 60px rgba(0,0,0,0.3)" }}>
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: 8, border: "1.5px solid var(--border)", borderRadius: 8, background: "var(--surface)", padding: "7px 12px", fontFamily: "ui-monospace, monospace", fontWeight: 800, fontSize: 13.5 }}>
          {invPrefix}<span style={{ color: "var(--muted)" }}>next</span>

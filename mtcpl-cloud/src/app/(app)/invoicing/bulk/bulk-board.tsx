...
/**
 * BulkBoard (Daksh) — the Bulk challans pool, styled like the Challans board:
 * a search bar (temple / challan no. / slab code), temple-wise sections that
 * DEFAULT COLLAPSED (auto-expand while searching), and a card per parked challan
 * with Download (delivery challan) + Send back (our confirm + spinner).
 */
...
export type BulkCard = { id: string; code: string; date: string; sourceDispatchId: string | null; search: string };
...
export function BulkBoard({ groups, total }: { groups: BulkGroup[]; total: number }) {
    ...
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
          {searchActive ? <>Results <span style={{ color: "var(--text)" }}>· {shown}</span></> : <>Bulk challans <span style={{ color: "var(--text)" }}>· {total}</span></>}
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: 12, padding: "30px 22px", textAlign: "center", color: "var(--muted)" }}>
          {searchActive ? <>No bulk challans match <strong>“{query}”</strong>.</> : <>No bulk challans. Send open challans here from the <Link href="/invoicing/challans" style={{ color: "var(--gold-dark)", fontWeight: 700 }}>Challans</Link> page.</>}
        </div>
      ) : (
              ...
              <div key={g.temple} style={{ border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", background: "var(--surface, #fff)" }}>
                ...
                {open && (
                  <div style={{ padding: 14, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(262px, 1fr))", gap: 12 }}>
                    {g.rows.map((c) => (
                      <div key={c.id} className="blk-card" style={{ border: "1px solid var(--border)", borderLeft: "4px solid #f59e0b", borderRadius: 12, background: "var(--surface, #fff)", padding: "12px 13px 13px", display: "flex", flexDirection: "column", gap: 9, animation: "blkLift 0.18s ease" }}>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 10.5, fontWeight: 800, color: "#92400e", background: "rgba(245,158,11,0.16)", borderRadius: 999, padding: "3px 9px", whiteSpace: "nowrap" }}>📦 In bulk</span>
                        </div>
                        ...
                        <div style={{ marginTop: "auto", paddingTop: 4, display: "flex", gap: 7, flexWrap: "wrap" }}>
                          {c.sourceDispatchId && (
                            <Link href={`/dispatch/${c.sourceDispatchId}/print`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, fontWeight: 700, padding: "8px 12px", border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg)", color: "var(--text)", textDecoration: "none" }}>🖨 Download</Link>
                          )}

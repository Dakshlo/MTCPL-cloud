...
export async function GET(req: NextRequest) {
  try {
    ...
    // Blank designations sort + group under this label everywhere.
    ...
    // Leftmost "Designation" column (col 1) is merged + rotated per group, like
    // the PF handler's own sheet. Everything else follows.
    const HEADERS = [
      "Designation",
    ...
    const MONEY = "#,##0.00";
    const WIDTHS = [7, 4, 22, 22, 20, 13, 18, 11, 12, 11, 11, 8, 9, 14, 11, 13, 14, 11, 15, 26];
    // Money columns after the +1 designation shift (Wage for PF … Actual Salary).
    const MONEY_FROM = 8, MONEY_TO = 19;
    ...
    // Data rows — track contiguous designation groups (rows are pre-sorted by
    // designation) so we can merge + rotate the leftmost column per group.
    const groups: Array<{ desig: string; start: number; end: number }> = [];
    let sr = 0;
    for (const r of rows) {
      sr += 1;
      ...
      row.eachCell((c, col) => {
        c.border = thin;
        c.font = { size: 9.5 };
        c.alignment = { vertical: "middle", horizontal: col <= 7 || col === 20 ? "left" : "right", wrapText: col === 20 };
        if (col >= MONEY_FROM && col <= MONEY_TO && typeof c.value === "number") c.numFmt = MONEY;
      });
      if (sr % 2 === 0) row.eachCell((c) => { c.fill = fill("FFF6EFF1"); });
      const g = groups[groups.length - 1];
      if (g && g.desig === r.designation) g.end = row.number;
    ...
    totalRow.eachCell((c, col) => {
      ...
      c.alignment = { vertical: "middle", horizontal: col <= 7 ? "left" : "right" };
      if (col >= MONEY_FROM && col <= MONEY_TO && typeof c.value === "number") c.numFmt = MONEY;
    });

    // Designation-wise subtotal mini-table (only when >1 designation) — each
    // designation's headcount + Actual-Salary-to-be-Paid total, then a grand total.
    ...
    if (byDesig.size > 1) {
      ws.addRow([]); // spacer
      const capRow = ws.addRow(["Designation-wise total (Actual Salary to be Paid)"]);
      ...
      const subHdr = ws.addRow(["Designation", "Employees", "Amount"]);
      ...
      const gt = ws.addRow(["TOTAL", rows.length, n2(rows.reduce((a, r) => a + r.net, 0))]);
      gt.eachCell((c, col) => {
        if (col > 3) return;
        c.font = { bold: true, size: 10 };
        c.fill = fill("FFEDE3E6");
    ...
    // Freeze the title + header rows AND the leftmost designation column.
